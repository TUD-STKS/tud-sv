...
dict_hit_no1 = {'Autor_in': author, 'Titel': title, 'Spieldauer': duration_s, 'Abtastfreq': fs_Hz, 'Spur links': track_left, 'Spur rechts': track_right}
...
print("Interpet: {}, Dauer: {} Sekunden".format(dict_hit_no1['Autor_in'], dict_hit_no1.get('Spieldauer')))
...
# Lösung
genre = "1a Kuschelrock"                       # Speichern Sie 'genre' unter dem Schlüssel 'Genre' in dict_mus_1
track_left_np = np.array([1, 0.458,0.23,-1])   # Überschreiben Sie 'Spur links' in dict_mus_1
track_right_np = np.array([1,0.5,0.02,1])      # Überschreiben Sie 'Spur rechts' in dict_mus_1
...
def interactive_signals(domain = 'Zeitsignal', signal1 =True, signal2 = True, signal3 = True, \
    ...
    for n in s:
        ...
        if type == 1:
            ...
            if domain == 's{}':
                print('Phase ist im Zeitbereich nicht möglich. Es wird der Betrag angezeigt.')
                m.append(abs(n))
            else:
                m.append(np.angle(n))   
        s = m
...
for seqIndex in range(numSegments):
    ...
    # Graphische Darstellung
    live_plot(t_s, s, tSegment_s, winsSegment, f_Hz, winS, seqIndex, xLim_max=50, yLim_max=90)
...
def wav_data(name, file):
    ...
    fs_Hz, s = wavfile.read(file)                   # Abtrastfrequenz, Signal
    if len(np.shape(s))>1:
...
for seqIndex in range(numSegments):
    ...
    # Grafische Darstellung:
    live_plot(t_s, audioSignal, tSegment_s, winsSegment, f_Hz, winS, seqIndex, xLim_max=fs_Hz/10, yLim_max=0.1)
